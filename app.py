...
running_procs = {}
TELEGRAM_BOTS = {}  # قاموس لتخزين عمليات البوتات
...
def start_telegram_bot():
    ...
    if not token:
        return jsonify({"success": False, "message": "التوكن مفقود"})
    ...
    try:
        ...
        TELEGRAM_BOTS[bot_name] = {"process": proc, "token": token, "log": log_path}
        
        # حفظ الإعدادات
        config = load_bots_config()
        config[bot_name] = {"token": token, "status": "running", "started_at": datetime.now().isoformat()}
...
def list_telegram_bots():
    ...
    for bot_name, bot_info in config.items():
        is_running = bot_name in TELEGRAM_BOTS
        bots_list.append({
            "name": bot_name,
            "status": "running" if is_running else "stopped",
            "started_at": bot_info.get("started_at"),
            "token": bot_info.get("token", "")[:20] + "..."
        })
